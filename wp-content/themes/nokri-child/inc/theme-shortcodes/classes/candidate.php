...
/************************************/
/* Ajax handler for Del Resume1 */
/************************************/
add_action('wp_ajax_delete_cand_resume1', 'nokri_delete_cand_resume1');
if ( ! function_exists( 'nokri_delete_cand_resume1' ) ) {
function nokri_delete_cand_resume()
{
	$user_crnt_id = get_current_user_id();
	if( $user_crnt_id == "" )
	...
	$attachmentid	=	trim($_POST['resume']);
	if( get_user_meta( $user_crnt_id, '_cand_resume1', true ) != "" )
	 {
	  $ids = get_user_meta( $user_crnt_id, '_cand_resume1', true );
	  $res =  str_replace($attachmentid, "", $ids);
	  $res =  str_replace(',,', ",", $res);
	  $img_ids= trim($res,',');
	   update_user_meta( $user_crnt_id, '_cand_resume1', $img_ids );
	 }	
...
{
	$result	=	array();
	$ids	=	get_user_meta ( get_current_user_id(), '_cand_resume1', true );
...
function nokri_view_application()
{
	...
	if (is_numeric($attachment_id)) 
	{
			...
			$label       =   esc_html__('Your have Applied Against CV', 'nokri' );
	} 
	else 
	{
...
if ( ! function_exists( 'nokri_job_alert_subscription' ) )
{
	...
	/* Not login */
	if($user_id == ''){echo "3"; die();}
	/* Not cand */
    if ( $type != '0' ){echo "4"; die();} 
    /*countries*/
	$cand_alert =	array();
	...
	if( $params['alert_type'] != "" )        {  $cand_alert[]	=	$params['alert_type'];  }
	if( $params['alert_experience'] != "" )  {  $cand_alert[]	=	$params['alert_category'];  }
	if( $params['alert_location'] != "" )    {  $cand_alert[]	=	$params['alert_location'];  } 
...
if ( ! function_exists( 'nokri_external_apply_package_base' ) ) 
{
	function nokri_external_apply_package_base() 
	{
		...
		/*demo check */
		$is_demo =  nokri_demo_mode();
		if($is_demo) {echo  '1|' . __( "Demo mode activated", 'nokri'); die(); }
		/* Is applying job package base*/
		$is_apply_pkg_base = ( isset($nokri['job_apply_package_base']) && $nokri['job_apply_package_base'] != ""  ) ? $nokri['job_apply_package_base'] : false;
		/*signin page*/
...
function nokri_send_email_new_rating($sender_id, $receiver_id, $rating = '', $comments = '') {
